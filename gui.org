...
get up an running with. Obviously, since phui is supposed to be delivered in
...
the .sm conversion, possibly other example files. It turns out that those are
not uniform in their return values. *But* a generalized interface already exists
in near-completeness! Enter, /phconvert.loader/! The functions is there return a
...
that I've likely working towards something that someone else already did. Doh!
